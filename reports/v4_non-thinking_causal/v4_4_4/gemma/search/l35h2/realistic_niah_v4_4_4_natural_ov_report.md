...
全局 intersection-union p=1，阈值 α=0.025。这里的全局 p 是四个预注册证据族 p 值的最大值；只要任一必要证据失败，完整机制主张就不成立。**本段结论：完整 natural-OV 主张为 False。**
...
|---|---:|---|---|
| natural_signal | 0.952849 | False | natural_carrier_count_slope p=9.53674e-07, mean=0.07997; natural_carrier_count_slope__candidate_minus_control_mean p=0.952849, mean=-0.006818 |
| pre_o_injection | 1 | False | injection_dose_slope p=1, mean=-0.00815; injection_dose_slope__candidate_minus_control_mean p=1, mean=-0.01338 |
| centered_removal | 0.994531 | False | removal_error_axis_minus_control p=0.910689, mean=-0.002517; removal_error_axis_minus_control__candidate_minus_control_mean p=0.967668, mean=-0.005169; removal_margin_axis_minus_control p=0.859655, mean=0.01337; removal_margin_axis_minus_control__candidate_minus_control_mean p=0.994531, mean=0.03651 |
| path_mediation | 0.995138 | False | donor_patch_transport p=0.356061, mean=0.0002674; donor_patch_transport__candidate_minus_control_mean p=0.995138, mean=-0.002781; mediation_control_minus_axis_block p=0.371373, mean=3.127e-05; mediation_control_minus_axis_block__candidate_minus_control_mean p=0.349543, mean=5.263e-05 |
...
自然 carrier/count slope=0.0800（95% CI 0.0706–0.0896，p=9.53674e-07）。真实 pre-O injection dose slope=-0.0081 expected-count/β（95% CI -0.0100–-0.0063，p=1）。**本段结论：自然 forward 中存在 count carrier，且真实 V→z→W_O channel 具有带符号充分性。**

相对同 span、等 post-O 范数的正交控制，natural-axis removal 使 absolute error 多增加 -0.0025（95% CI -0.0060–0.0008，p=0.910689），使 correct-count margin 多下降 0.0134（95% CI -0.0083–0.0363，p=0.859655）。**本段结论：centered z-space removal 支持该自然 channel 对计数是必要的。**

Donor-z patch 的 normalized transport=0.0003（95% CI -0.0008–0.0016，p=0.356061）；相对正交控制，自然轴阻断额外消除 0.0000，约占 donor transport 的 11.7%（p=0.371373）。**本段结论：同一自然 OV 轴部分介导 donor effect，而不是只对任意 perturbation 敏感。**

## 基线与数据审计

无干预候选答案准确率=0.385，expected-count MAE=1.25，确认样本=200。审计 all_checks_pass=True；observed rows={'dataset_rows': 300, 'confirmation_seed_shards': 20, 'natural_rows': 1000, 'directed_rows': 2100, 'mediation_rows': 900}。**本段结论：因果结果是在模型原始计数行为与完整 seed grid 上计算，且产物计数通过审计。**

## Nested-K 二级结果

| K | heads | natural Holm p | injection Holm p | removal Holm p |
|---:|---|---:|---:|---:|
| 1 | 2 | 9.53674e-07 | 1 | 0.910689 |
