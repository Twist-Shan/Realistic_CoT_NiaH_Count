...
- **Partially answered:** the decomposition of prompt noise, including identity/context/position; why prompt readout appears early while answer executability appears late; and the integrated distributed-evidence→retrieval→late-state chain. The adjacent arrows are supported, but they have not yet been combined in one nested-intervention forward.
- **Open:** only the optional head-level test of whether the early update is a classical induction-head circuit.
- **Falsified:** the opening two-sentence counting-definition cue is not necessary for an ordered shallow running geometry. This does **not** test removal of the counting question or numeric-output instruction.
...
**Partially established as a stage-wise distributed chain, not as transport of one fixed three-dimensional axis.** Dense full-span restoration shows that early distributed prompt evidence remains reusable; the restoration-induced attention response shows that repairing it reconfigures later broad retrieval; frozen retrieval-subspace interventions establish a local natural aggregation window; late answer-state patch/removal and Qwen OV / Gemma residual mediation connect the downstream state to output. These adjacent arrows are already supported by matched controls, but they were estimated in separate experiments rather than jointly blocked within the same seed-count forward.
...
#### Planned experiment 19: same-forward nested serial mediation
...
**This experiment has not yet been run.** Passing the ordered criteria would upgrade the evidence from multi-experiment triangulation to same-forward partial serial mediation. Failure at one stage would indicate bypass, redundancy, or basis mismatch, not that prompt evidence is irrelevant.

#### Planned experiment 22: classical induction-head identification
...
The evidence supports a representational-to-causal transition rather than a single counter being carried unchanged through the network. Needle ends form an early, low-dimensional but noisy and position-confounded running-index trajectory. Raw needle evidence is essential and causally reusable as a complete span, whereas the fitted endpoint rank-3 direction is not a strong localized register. Repairing early spans changes later broad retrieval; local retrieval-subspace interventions identify a partial aggregation pathway; late answer-query states become sufficient and direction-specifically necessary for the generated count. Qwen additionally supports a localized L28 OV writer, while Gemma supports a more distributed late residual mediator. The highest-value remaining mechanism experiment is the same-forward serial mediation above; classical induction-head identification and token-level QK/V decomposition are optional refinements.

## Planned follow-up experiments

The preregistered purposes, matched-pair construction, interventions, controls, readouts, decision rules, execution order, and GPU plan for the remaining prompt-to-answer mechanism experiments are recorded in [`plans/nonthinking-followup-experiment-log-20260813.md`](../plans/nonthinking-followup-experiment-log-20260813.md). Before running any experiment, append its exact frozen artifacts, hashes, seed/stimulus manifest, and commands without replacing earlier hypotheses or results. Experiment 19 is the highest-priority mechanistic completion; question 23 is a conditional medium-priority noise-source decomposition; question 22 is an optional low-priority micro-circuit test. Question 21's opening-definition necessity claim is falsified, whereas question 24 alone is closed as an out-of-scope abstract-counter claim.
