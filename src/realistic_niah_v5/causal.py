...
def ranked_bank_with_layer_profile(
    ...
    *,
    ...
    This is a diagnostic treatment, not a random control: scores come from the
    supplied ranking while the layer histogram is frozen from a reference
    bank selected at another query site.
    """
    ...
    if missing:
        raise ValueError(f"Head ranking is missing {missing}")
    normalized = {int(layer): int(count) for layer, count in layer_profile.items()}
