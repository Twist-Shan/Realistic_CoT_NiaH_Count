...
def freeze_v31_grid(
    *,
    ...
    if resolved.passage_lengths != PASSAGE_LENGTHS:
        raise ValueError("V3.1 freezing requires the registered length grid")
    if resolved.needle_counts != NEEDLE_COUNTS:
        raise ValueError("V3.1 freezing requires the registered needle grid")
    if resolved.seeds != SEEDS:
        raise ValueError("V3.1 freezing requires the registered paired seeds")
    if (
        resolved.insertion_depth_min_fraction != INSERTION_DEPTH_MIN_FRACTION
        or resolved.insertion_depth_max_fraction != INSERTION_DEPTH_MAX_FRACTION
    ):
        raise ValueError("V3.1 freezing requires the 5%-95% depth interval")
    if canonical_tokenizer_revision not in (None, CANONICAL_TOKENIZER_REVISION):
        raise ValueError("V3.1 canonical tokenizer revision is immutable")
    if resolved.canonical_tokenizer_revision not in (
        None,
        CANONICAL_TOKENIZER_REVISION,
    ):
        raise ValueError("V3.1 freeze spec has an unregistered tokenizer revision")
    resolved = replace(
        resolved,
        canonical_tokenizer_revision=CANONICAL_TOKENIZER_REVISION,
        tokenizer_cache_dir=(
...
def audit_v31_grid(
    *,
    ...
    registered = (
        ...
        and saved.get("canonical_tokenizer_revision") == CANONICAL_TOKENIZER_REVISION
    ...
    if int(report.get("rows_checked", -1)) != EXPECTED_STIMULI:
        ...
        report.setdefault("errors", []).append(
            f"V3.1 requires {EXPECTED_STIMULI} stimuli, "
            f"found {report.get('rows_checked')}"
        )
    if not registered:
        report["passed"] = False
        report.setdefault("errors", []).append(
            "Frozen manifest does not match the registered V3.1 grid"
        )
    return report
