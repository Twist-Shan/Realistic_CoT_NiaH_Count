...
The main report no longer presents this as a primary section. Appendix D shows the Qwen and Gemma running-index versus pre-answer comparisons as interpretive diagnostics rather than headline results. It includes interactive PCA3 views at each endpoint's discovery-selected SNR layer by default, with every layer and both full/confirmation cohorts available. The display bases are fitted independently per endpoint/layer, so absolute axes and scale are not a contraction estimand. Qwen retains its mixed-metric boundary; Gemma's four same-direction covariance metrics are shown as broader supportive evidence, not a strict paired consolidation test.
