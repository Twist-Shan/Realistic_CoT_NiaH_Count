...
### 2026-09-02 experiment 19 Qwen statistical-retention repair

- **Recovery scope.** This repair did not rerun the model or change the registered cohort, interventions, layers, heads, or bases. The complete Qwen tree was recovered read-only from `/lambda/nfs/CoT-Non-thinking-v4/runs/nonthinking_v445_exp19_serial_mediation_20260814` into `work_remote_snapshots/q19_serial_mediation_20260814_recovered_20260902`. All nine Qwen files match the persistent copy by SHA-256; the persistence audit remains `PASS` with 9 files and 6,921,379 bytes on both sides. The recovered detail and broad tables contain exactly 1,100 and 7,700 rows, with SHA-256 `580afeff2f8dc2a0f2e968791d7ae782f9bce55a581cd7d90c352f9d5dd8f926` and `fc0ecc290cd2daf8e9913ef33c16484ff0b1047cb9ddfb2a3df88ffadc723dc1`.
- **Paired-table identity.** Reanalysis regenerated the 100-row `paired_serial_effects.csv` byte-for-byte: both the recovered original and regenerated file have SHA-256 `43bb6674a9e359b5dcd1a73586779edf145b94fc64e74ed3afc37fc231ce1d5d`. The corresponding V2 summary and analysis audit have SHA-256 `bbc6b71171799f9c84c4de055ca097d8d977faee0538975736a6fcf0d7a413f8` and `dc12c7095af06425c14795a39b474425f86e54b47ba33c885f89871e7371e7da`. The analyzer and frozen experiment configuration have SHA-256 `600cd059040b697874ec07af8488b618f74c4d276f79f033b1871ad73919acd7` and `b6f3ac89c551468b0362ee07593db2cedef5f908457fb8d2238c9f5d9cd244df`.
- **Corrected inference unit.** The V2 analysis first averages the ten registered counts within each seed, then averages the ten seed means. Uncertainty uses 10,000 seed-cluster percentile-bootstrap draws, and the secondary test enumerates all $2^{10}=1{,}024$ sign assignments over seed means. The point estimates agree with the legacy balanced-panel means for all 14 metrics (maximum absolute difference $3.55\times10^{-15}$), but the legacy seed--count bootstrap intervals on lines above are superseded and must not be used for inference.
- **Seed-cluster results.** Source repair is 2.6744 counts (95% CI 2.4416--2.9188), retrieval mediation 0.3268 (0.2268--0.4346), late mediation 1.1185 (0.8858--1.3757), the retrieval-by-late interaction -0.3823 (-0.4732 to -0.2963), and remaining repair 1.4771 (1.2376--1.7217). Each two-sided exact seed sign-flip test gives $p=0.001953125$. These results retain the registered conclusion of ordered partial serial mediation; the negative interaction and positive remaining repair continue to limit the claim to a non-unique, non-exhaustive pathway.
...
### 2026-08-15 experiment 23 V2 audited completion, result, and campaign close

- **Version and exact coverage.** The formal V2 assay used executable commit `d699e0754f9ebdaa07119cff80121ee3e6713a22` and frozen stimulus SHA-256 `da4dd86142eb8a07f9a7e53497efd3375184c8e68367d4db994370fcb331f090`. Each model completed exactly 240 factorial forwards (160 discovery, 80 confirmation), 240 state files containing 2,400 endpoint states, 400 outside-context rows and state files, and 100 edge-audit units. Completion markers and 10,000-draw analysis audits report `PASS`; sequence/query preservation, matched donors and carriers, frozen source registration, finite states/readouts, one source-hook application per causal row, positive equal candidate/control edge counts, distinct keys, and exact distance-bin pairing all pass. No GPU runner remained after completion.
- **Controlled factorial result.** The discovery-fitted rank-3 centroid bases retain 0.988073 of Qwen and 0.979015 of Gemma count-centroid energy. On confirmation seeds, however, the full identity/context/position factorial models have held-out $R^2=-0.022126$ and $-0.089317$. Qwen incremental $\Delta R^2$ values for identity/context/position are -0.000105/-0.000408/+0.017461; Gemma values are +0.003141/+0.000231/-0.000721. Because the full models do not beat the intercept baseline, these small differences are interpreted only as deformation under the registered manipulations, not as a decomposition of natural manifold variance or evidence for a compact nuisance-factor model.
- **Outside-context matched causal result.** The frozen source heads are Qwen L5H13 and Gemma L5H1. Positive candidate-minus-control expected-error damage was required against both controls. Qwen candidate-minus-distance is +0.012274 counts (95% seed-bootstrap CI -0.005436 to +0.026461) and candidate-minus-attention-mass is +0.003058 (-0.009971 to +0.016300). Gemma values are -0.002216 (-0.009881 to +0.005576) and +0.003861 (-0.005776 to +0.012924). Thus `candidate_exceeds_both_controls=false` for each model. Broad-score contrasts are Qwen +0.000386/+0.000512 and Gemma -0.000073/-0.000224 versus distance/attention controls; correct-margin intervals are not jointly directionally specific. Late states were captured and passed finite/state-count audits, but the registered scientific gate is the paired candidate-versus-both-controls behavioral specificity, which fails.
- **Scientific verdict and scope.** The completed factorial does not yield a generalizing identity/context/position explanation of prompt scatter, and the selected outside-halo natural contributions are not specifically more necessary than both matched controls. This falsifies the registered privileged-source package for question 23. It does not reject distributed outside-context synergy, a different discovery registry, or a fully renormalized QK counterfactual. The implemented intervention subtracts frozen natural $\alpha V$ at the answer-query source-head pre-output-projection slice and cannot establish pathway uniqueness.
- **Persistence audit.** Node-local model outputs and root logs were copied without deletion to `/lambda/nfs/CoT-Non-thinking-v4/runs/nonthinking_v445_exp23_noise_factorial_v2_20260815` with model-isolated manifests. Qwen source/destination match at 750 files, 19,695,962 bytes, manifest SHA-256 `aa66116a830d29a012608b2db408f40c3fbb07dfbd4874f2107c8d8d6f7af8c8`; Gemma matches at 750 files, 13,637,381 bytes, SHA-256 `e24fd4fcfe1fd4def297125433f28ebbe7f7fe27da997a603f5bdf0229448460`. Both persistence audits report `PASS`; node-local evidence was retained.
- **Report integration and queue status.** `reports/NiaH_Non-thinking_report.html` now places the mechanism first, integrates experiments 19/22/23 with exact definitions, axes, intervals and scope, and audits all 25 extension questions as 12 verified, 9 falsified, 3 partially answered, 0 open and 1 closed. Its static validator passes 25 audit rows, all internal anchors, 38 labeled inline SVGs, 17 collapsible sections and zero external assets. The strict sequential queue 19→22→23 is complete; there is no remaining required GPU experiment in this campaign.

### 2026-08-17 layerwise full-span state-deformation audit

- **Purpose and frozen design.** This follow-up directly measures the missing input→state response without choosing a PCA direction. Each model uses confirmation seeds 1254--1263 × counts 1--10, for 100 prompt units. Every unit has three pure forwards: clean, all-active-needle replacement, and a non-overlapping ordinary-passage replacement matched exactly on the per-span token-length vector and total token budget. Sequence length and answer-query position are preserved. Full residual vectors are read only at registered target positions after every block: Qwen L0--35 and Gemma L0--41. The primary layerwise score is needle raw-RMS deformation minus ordinary raw-RMS deformation; relative-RMS and mean tokenwise cosine-distance differences are secondary. The inference unit is seed after averaging ten counts, with 50,000 seed bootstraps and the exact two-sided $2^{10}$ sign-flip test. Reusable windows were independently frozen from dense restoration at Qwen L0--20 and Gemma L0--16.
- **Execution and structural audit.** The versioned node-local root is `/home/ubuntu/runs/nonthinking_v445_span_state_deformation_20260817`. Qwen completed exactly 3,600 unique seed--count--layer rows and Gemma exactly 4,200; all key, finite-value, token-budget, sequence/query, condition-registry, and completion audits report `PASS`. The combined analyzer produced exactly 78 layer rows and `analysis_audit.json` reports `PASS`. The formal source snapshot manifest is `608ff4b677ba891e2cf20138ca61c04ab2e1cb4f0847a06ab4fe92ae1117f671` (67 files).
- **Primary result.** In the independently frozen behavior-reuse window, raw-RMS specificity is Qwen +0.51246 (95% seed-bootstrap CI -0.02076 to +1.15093, exact sign-flip $p=0.14844$) and Gemma +0.01021 (-0.00597 to +0.03747, $p=0.83594$). Thus neither model supports the strong whole-window primary claim that active-needle replacement causes consistently larger raw full-vector deformation than an equal-budget ordinary replacement. Pointwise nominal-positive raw layers are Qwen L2--4 and L35, and Gemma L0--2, L4--6, L16--24, L29--30, and L39--41; these pointwise tests are uncorrected and serve only as localization.
- **Scale-normalized secondary and interpretation.** Gemma supports positive reusable-window relative-RMS specificity +0.02400 (0.02089 to 0.02690, $p=0.00195$) and cosine specificity +0.00785 (0.00485 to 0.01247, $p=0.00195$). Qwen's corresponding whole-window tests are not supported, although localized mid-layer normalized effects occur. Most importantly, Gemma retains geometric clean/corrupt differences through L17--24 even though behavior restoration drops at L16→17. State deformation and behavioral reusability are therefore distinct: an input perturbation can keep propagating after the original prompt-side evidence has missed the downstream use window. This experiment directly closes the input→state response measurement, while dense clean-state restoration remains the decisive state→count causal-use test; state difference alone is not a mediator proof.
- **Persistence.** The complete run was copied without node-local deletion to `/lambda/nfs/CoT-Non-thinking-v4/runs/nonthinking_v445_span_state_deformation_20260817`. The persistence audit reports `PASS`, 162/162 verified files, 10,998,707 bytes, and manifest SHA-256 `77032e1e029305b6fbc15b53aa654fc368069876cac0d3b67223fbf8b1a3c240`.
- **Report integration.** Section 3.3 of `reports/NiaH_Non-thinking_report.html` now separates the directly measured input→state response from the state→count restoration test, includes four layerwise panels with matched-control definitions, axes, exact uncertainty rules and a worked count-6 example, and removes all stale “state-effect not yet measured” language. The rebuilt static report has 31 figure titles and exactly 31 pre-figure reading guides, 56 labeled inline SVGs, no broken images, and no browser console warnings/errors.

### 2026-08-17 held-out running-index state-retention audit

- **Question and frozen selection rule.** The raw full-vector deformation assay above asks only whether two states differ in magnitude; it does not test whether the perturbed state still belongs to the correct running-index organization. Before inspecting outcomes, this follow-up froze ten candidate summaries/model and a display priority: endpoint correct-centroid distance, span-mean correct-centroid distance, endpoint/span-mean nearest-centroid margin and accuracy, followed by endpoint/span-mean relative RMS and cosine deformation. Selection required cross-model held-out support after within-model, within-family Holm correction; every frozen metric is retained in the report, so the displayed metric was not chosen by its smallest p-value.
- **Data, split, and geometry.** Final gold count is fixed at 10 so that each prompt supplies running indices 1--10. Discovery clean seeds 1234--1253 (20 prompts/model) freeze, separately at every post-block layer and for endpoint/span-mean pooling, the ten running-index centroids and their RMS class radii. Confirmation seeds 1254--1263 (10 prompts/model) use three length/query-preserving arms: clean, all-active-needle corruption, and a non-overlapping ordinary-passage corruption matched exactly on the per-span token-length vector and total token budget. Coverage is all layers (Qwen L0--35; Gemma L0--41), with independently frozen reusable windows Qwen L0--20 and Gemma L0--16.
- **Primary estimand.** For state $h$ and its correct running-index centroid $\mu_k$, standardized correct distance is $z_k=\lVert h-\mu_k\rVert_2/r_k$, where $r_k$ is the discovery RMS radius of class $k$. The needle-specific effect is $z_{needle}-z_{ordinary}$, averaged over the ten indices and frozen layers within each confirmation seed. Positive values mean needle corruption moves the state farther from the correct clean class than an equal-budget ordinary corruption. Inference uses 50,000 seed bootstraps, exact two-sided $2^{10}$ sign flips, and Holm correction within the six-member retention or four-member deformation family for each model.
- **Exact coverage and audit.** The formal root is `/home/ubuntu/runs/nonthinking_v445_state_retention_20260817`, executable repository SHA `22f828bef736bfa4c91e292a5f4c31e838e3cc4b`, and code-snapshot manifest SHA-256 `35b0bd510ddce481e2de1b22311383063a6e85c0e2e91b55cbdff19aaa3ac673`. Qwen has exactly 720/720 layer rows and 7,200/7,200 occurrence rows; Gemma 840/840 and 8,400/8,400. Both model audits, both analyses, and the joint audit report `PASS`; the analyzer produced exactly 780 layerwise and 20 frozen-window statistics.
- **Primary result.** Endpoint correct-distance window levels are Qwen clean 1.016, ordinary 1.009, needle 7.969 and Gemma clean 0.963, ordinary 0.961, needle 4.777. The preregistered needle-minus-ordinary effects are Qwen +6.96065 class radii (95% seed-bootstrap CI 6.80652--7.13500) and Gemma +3.81629 (3.64712--3.92594); both have exact sign-flip $p=0.001953$ and retention-family Holm-adjusted $p=0.011719$. The result is not only an L0 spike: endpoint specificity remains +2.24 at Qwen L20 and +1.31 at Gemma L16.
- **Sensitivity and interpretation.** All ten frozen indicators are positive and Holm-significant in both models. Span-mean correct-distance effects are +2.76533/+2.37704; endpoint margin damage +1.29683/+0.69805; endpoint accuracy damage +0.32095/+0.11765; endpoint relative-RMS specificity +1.03055/+0.57301; and endpoint cosine specificity +0.53377/+0.18157 for Qwen/Gemma. Clean nearest-centroid accuracy itself is only about 0.40/0.22 at the endpoint, confirming that the ten full state clouds overlap; therefore continuous standardized distance, not classifier accuracy, is the primary display. Together with the matched ordinary arm, this directly supports the arrow needle input→count-organized span state. It does not by itself prove state→output mediation; that second arrow remains established by full-span clean-state restoration against equal-budget ordinary-state restoration.
- **Persistence and report integration.** The complete 187-file run, code snapshot, centroid banks, audits, and analyses were copied without deleting node-local data to `/lambda/nfs/CoT-Non-thinking-v4/runs/nonthinking_v445_state_retention_20260817`. Source/destination path-size manifests both hash to `673338d14375c11cda62695079f91112a2747a0ca6bbbb419c5981cb02bf767d`; per-file content-SHA aggregates both hash to `b6bb5acd4114a9434f469a3a638dbaab65c079256abdf9e3006eec7e0e916351`. `reports/NiaH_Non-thinking_report.html` now includes the complete metric definition, worked example, absolute condition levels, layerwise endpoint/span-mean curves, multiplicity rule, all ten outcomes, and the explicit boundary between input→state organization and state→count causal use.
