#!/usr/bin/env python3
"""Build a compact one-row Qwen3 accuracy-by-length figure.

This is a presentation derivative of the audited V3.2 empirical-law output.
It does not refit any model: observed points and slot-specific selected-law
curves are read from the same frozen tables used by the full report.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cells",
        type=Path,
        default=tables / "cell_outcomes.csv.gz",
    )
    parser.add_argument(
        "--coefficients",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=root / "reports" / "NiaH_accuracy_Qwen3_first_row_compact.png",
    )
    parser.add_argument(
        "--pdf-output",
        type=Path,
        default=root / "reports" / "NiaH_accuracy_Qwen3_first_row_compact.pdf",
    )
...
def build_figure(
    ...
    for slot_index, (slot, ax) in enumerate(zip(QWEN_SLOTS, axes)):
        ...
        for mode_index, mode in enumerate(MODES):
            ...
            for n_index, (target_count, color) in enumerate(zip(n_levels, colors)):
                observed = (
                    cells[
                        cells["comparison_slot"].eq(slot)
                        & cells["prompt_mode"].eq(mode)
                        & cells["N"].eq(target_count)
                    ]
                    .set_index("L")
                    .reindex(l_levels)["parsed_exact_accuracy"]
                    .to_numpy(float)
                )
                predicted = fitted[mode][slot_index, n_index, :]
                ax.plot(
                    l_levels,
                    predicted,
                    color=color,
        ...
        apply_all_l_ticks(ax, l_levels, fontsize=7.3)
        ...
        ax.set_xlabel("Passage length L (tokens; log positions)", fontsize=8.2, labelpad=7)
    ...
    count_handles = [
        plt.Line2D([], [], color=color, linewidth=2.0, label=f"N={target_count}")
        for target_count, color in zip(n_levels, colors)
    ]
    ...
    fig.suptitle(
        "Exact-count accuracy by passage length · Qwen3 family",
        x=0.052,
        y=0.975,
        ha="left",
        fontsize=13.2,
