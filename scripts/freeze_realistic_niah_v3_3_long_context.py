...
from realistic_niah_v3_3_long_context.stimuli import (
    audit_long_context_grid,
    default_freeze_spec,
    freeze_long_context_grid,
)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze, fully audit, and seal the V3.3 long-context grid."
    )
    ...
    paths = freeze_long_context_grid(
        output_dir=output,
        spec=spec,
        tokenizer_cache_dir=args.cache_dir,
        overwrite=args.overwrite,
    )
    audit = audit_long_context_grid(
    ...
    if not audit["passed"]:
        raise RuntimeError(
            f"V3.3 long-context frozen-grid audit failed: {audit['errors']}"
        )
    seal = seal_frozen_dataset(output)
    seal_path = output / "dataset_seal.json"
    print(
        json.dumps(
            {
