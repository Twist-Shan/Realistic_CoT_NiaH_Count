...
case "${run_root}" in
  */runs/realistic_niah_v2/reasoning_models_extension_*)
    ;;
  *)
    echo "RUN_ROOT must end in reasoning_models_extension_*" >&2
    exit 2
    ;;
esac
...
worker_prefix="rniah-reason-${run_tag}-gpu"
finalizer_session="rniah-reason-${run_tag}-final"
...
for gpu_id in $(seq 0 "$((workers - 1))"); do
  session="${worker_prefix}${gpu_id}"
  command="cd $(printf '%q' "${repo}") && exec bash scripts/run_realistic_niah_reasoning_models_extension_worker.sh $(printf '%q' "${run_root}") ${gpu_id}"
  tmux new-session -d -s "${session}" "${command}"
done
finalizer_log="${run_root}/orchestration/finalizer.log"
finalizer_command="cd $(printf '%q' "${repo}") && exec bash scripts/finalize_realistic_niah_reasoning_models_extension.sh $(printf '%q' "${run_root}") >> $(printf '%q' "${finalizer_log}") 2>&1"
tmux new-session -d -s "${finalizer_session}" "${finalizer_command}"

echo "Launched reasoning-model extension with ${workers} worker(s)."
