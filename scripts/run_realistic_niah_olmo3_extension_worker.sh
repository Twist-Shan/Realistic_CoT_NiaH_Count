...
plan_tsv="${run_root}/orchestration/olmo3_extension_shards.tsv"
...
engine_limits_for() {
  case "$1" in
    Olmo3-7B-Think) echo "6 4" ;;
    Olmo3-7B-Instruct) echo "8 8" ;;
    *)
      echo "No OLMo engine limits registered for $1" >&2
      return 2
      ;;
  esac
}
...
archive_previous_attempt_if_safe() {
  ...
  # A failed marker proves the previous wrapper exited. A claim from another
  # host is also stale because this extension is scoped to one rented instance.
  # Preserve both records before allowing checkpoint-based continuation.
...
# Avoid simultaneous model/cache initialization when multiple workers launch.
sleep "$((gpu_id * 5))"
