...
from realistic_niah_v4_4_3.pipeline import initialize_campaign
from realistic_niah_v4_4_3.set_analysis import analyze_set_campaign, audit_set_campaign
from realistic_niah_v4_4_3.set_pipeline import run_set_model_campaign
...
DEFAULT_NAMESPACE = "/lambda/nfs/CoT-Non-thinking-v4/runs/v4_4_3_ov_causal"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run isolated V4.4.3 small-head-set OV causal tests."
    )
    parser.add_argument(
        "--stage",
        required=True,
    ...
    parser.add_argument(
        "--config", default="configs/realistic_niah_v4_4_3_set.json"
    )
...
def _write_state(run_root: Path, state: str, **detail: object) -> None:
    ...
    atomic_json(
        ...
        {
            "schema_version": "realistic_niah_v4_4_3_set_campaign_status_v1",
            "state": state,
...
def main() -> int:
    ...
    config = V443SetConfig.from_json(config_path)
    ...
    if args.stage in {"initialize", "campaign"}:
        ...
        raise FileNotFoundError("Run root is not initialized")
    ...
    if args.stage == "campaign":
        _write_state(run_root, "RUNNING", stage="set_model_campaigns")
        models = [args.model] if args.model else list(config.model_labels)
        results = {}
        for model in models:
            results[model] = run_set_model_campaign(
                source_run_root=args.source_run_root,
                run_root=run_root,
                model_label=model,
                config=config,
        ...
        if set(models) == set(config.model_labels):
            analysis = analyze_set_campaign(run_root, config=config)
            audit = audit_set_campaign(run_root, config=config)
            _write_state(
                run_root,
            ...
            if not audit["complete"]:
                raise RuntimeError("V4.4.3-Set final audit failed")
        print(json.dumps(results, ensure_ascii=False, indent=2))
        return 0
    if args.stage == "analyze":
        result = analyze_set_campaign(run_root, config=config)
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0
    if args.stage == "audit":
        result = audit_set_campaign(run_root, config=config)
        print(json.dumps(result, ensure_ascii=False, indent=2))
