...
ROUTING="$CODE_ROOT/configs/realistic_niah_v5_qwen_unified_p0_k110_causal_routes_dev.json"
SOURCE_WRITES="$RUN_ROOT/source_attention_p0_all_grammars_discovery_v1"
PLAN="$RUN_ROOT/causal_plan_p0_all_grammars_k110_fullpanel_dev_v1"
OUTPUT="$RUN_ROOT/head_behavior_unified_p0_pooledbank_k110_discovery_smoke30_v1"
LOG="$RUN_ROOT/logs/qwen_unified_p0_k110_smoke_supervisor.log"
COMPLETE="$RUN_ROOT/qwen_unified_p0_k110_smoke_complete.json"

mkdir -p "$RUN_ROOT/logs" "$RUN_ROOT/locks" "$CACHE_DIR"
exec 9>"$RUN_ROOT/locks/qwen_unified_p0_k110_smoke.lock"
if ! flock -n 9; then
  echo "another unified-P0 Qwen K110 smoke supervisor owns the lock" >&2
  exit 75
fi
...
for name in sorted({str(row.get("routed_target_grammar_class")) for row in selected}):
    ...
    grammar[name] = {
        "anchors": len(selected_subset),
        "selected_failure_rate": sum(map(failed, selected_subset)) / len(selected_subset),
        "random_failure_rate": sum(map(failed, random_subset)) / len(random_subset),
    }
