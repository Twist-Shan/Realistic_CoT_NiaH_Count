#!/usr/bin/env python3
"""Run the fixed Gemma terminal-token -> state -> answer bridge."""
...
def main() -> None:
    ...
    if any("selection_rank" in row for row in anchors):
        raise ValueError("Terminal token-state bridge forbids selection_rank")
    ...
    if len(rows) != expected or len({int(row["seed"]) for row in rows}) != expected:
        raise ValueError("Terminal token-state bridge changed the fixed seed contract")
    rows.sort(key=lambda row: int(row["seed"]))
    plan_core = {
        "schema_version": "realistic_niah_v5_terminal_token_state_row_plan_v1",
    ...
    if plan_path.exists():
        if json.loads(plan_path.read_text(encoding="utf-8")) != plan:
            raise ValueError("Existing terminal token-state row plan changed")
    else:
    ...
    for index, row in enumerate(rows, start=1):
        shard = shards / f"{_safe_stem(row['request_id'], 'terminal_token_state')}.jsonl"
        ...
        completed += 1
        print(f"[terminal-token-state] {index}/{len(rows)}", flush=True)
    _atomic_json(
