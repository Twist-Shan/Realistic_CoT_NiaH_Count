...
ROUTING="$CODE_ROOT/configs/realistic_niah_v5_qwen_unified_p0_causal_routes_dev.json"
SOURCE_WRITES="$RUN_ROOT/source_attention_p0_all_grammars_discovery_v1"
PLAN="$RUN_ROOT/causal_plan_p0_all_grammars_k125_fullpanel_dev_v1"
OUTPUT="$RUN_ROOT/head_behavior_unified_p0_pooledbank_k125_discovery_smoke30_v1"
LOG="$RUN_ROOT/logs/qwen_unified_p0_k125_smoke_supervisor.log"
COMPLETE="$RUN_ROOT/qwen_unified_p0_k125_smoke_complete.json"

mkdir -p "$RUN_ROOT/logs" "$RUN_ROOT/locks" "$CACHE_DIR"
exec 9>"$RUN_ROOT/locks/qwen_unified_p0_k125_smoke.lock"
if ! flock -n 9; then
  echo "another unified-P0 Qwen smoke supervisor owns the lock" >&2
  exit 75
fi
...
random = [row for row in rows if row["condition"] == "layer_matched_random"]
grammar = {}
for name in sorted({str(row.get("routed_target_grammar_class")) for row in selected}):
    subset = [row for row in selected if str(row.get("routed_target_grammar_class")) == name]
    grammar[name] = {
        "anchors": len(subset),
...
payload = {
    "schema_version": "realistic_niah_v5_qwen_unified_p0_smoke_v1",
    "status": "PASS",
    ...
    "random_failure_rate": sum(map(failed, random)) / len(random),
    "selected_minus_random_failure": (
        sum(map(failed, selected)) / len(selected)
        - sum(map(failed, random)) / len(random)
    ),
    "grammar_selected_failure": grammar,
    "plan_sha256": manifest["plan_sha256"],
    "routing_sha256": manifest["anchor_routing_sha256"],
    "registry_sha256": hashlib.sha256(registry.read_bytes()).hexdigest(),
}
complete.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
