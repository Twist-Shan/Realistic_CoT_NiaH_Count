...
case "${run_root}" in
  */runs/realistic_niah_v2/reasoning_models_extension_*)
    ;;
  *)
    echo "Refusing unexpected reasoning-model extension root: ${run_root}" >&2
    exit 2
    ;;
esac
...
while true; do
  ...
  if [[ "${#failed[@]}" -gt 0 ]]; then
    echo "Reasoning-model extension has failed shard markers:" >&2
    printf '%s\n' "${failed[@]}" >&2
    exit 1
  fi
  completed="$(
  ...
  printf '%s reasoning-model extension: %s/%s shards complete\n' \
...
PYTHONPATH=src "${python_bin}" \
  scripts/merge_realistic_niah_reasoning_models_extension_shards.py \
  --run-root "${run_root}"
"${python_bin}" -c \
  'import json,sys; a=json.load(open(sys.argv[1],encoding="utf-8")); assert a["passed"] is True; assert a["requests"] == a["unique_request_ids"] == 10000' \
  "${run_root}/orchestration/final_shard_audit.json"
echo "Reasoning-model extension final audit passed: ${run_root}"
