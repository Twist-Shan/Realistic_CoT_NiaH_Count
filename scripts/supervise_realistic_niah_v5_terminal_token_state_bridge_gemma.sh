...
RUNNER="$ROOT_DIR/scripts/run_realistic_niah_v5_terminal_token_state_bridge.py"
ANALYZER="$ROOT_DIR/scripts/analyze_realistic_niah_v5_terminal_token_state_bridge.py"
...
OUTPUT_ROOT="$ROOT_DIR/work/v5_native_count_stream/terminal_token_state_bridge_gemma_20d10c_20260821_v1/$MODEL"
COMPLETE="$OUTPUT_ROOT/terminal_token_state_complete.json"
LOG="$OUTPUT_ROOT/logs/terminal_token_state.log"
mkdir -p "$OUTPUT_ROOT/logs" "$OUTPUT_ROOT/locks"

exec 9>"$OUTPUT_ROOT/locks/terminal_token_state.lock"
if ! flock -n 9; then
  echo "another Gemma terminal-token-state supervisor owns the lock" >&2
  exit 3
fi
...
assert len(rows) == 30
assert len({int(x["seed"]) for x in rows}) == 30
assert all("selection_rank" not in x for x in rows)
assert all(x["write_edge_outcome_blind"] is True for x in rows)
PY
...
value = {
    "schema_version": "realistic_niah_v5_terminal_token_state_complete_v1",
    "status": "PASS",
    ...
    "claim_scope": "terminal_item_token_content_to_fullspan_state_to_answer",
    "targeted_retrieval_linkage": (
        "modular: pair with frozen Gemma Top-6 city/item retrieval endpoint"
    ),
    "discovery": discovery,
    "confirmation": confirmation,
    "complete_terminal_token_state_pass": bool(
        discovery["terminal_token_state_mediation_pass"]
        and confirmation["terminal_token_state_mediation_pass"]
    ),
}
