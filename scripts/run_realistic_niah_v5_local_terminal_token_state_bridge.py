#!/usr/bin/env python3
"""Run the fixed Gemma local terminal-token -> state -> answer bridge."""
...
def main() -> None:
    ...
    parser.add_argument("--model", choices=["Gemma4-E4B"], required=True)
    ...
    if any("selection_rank" in row for row in anchors):
        raise ValueError("Local terminal token-state bridge forbids selection_rank")
    ...
    if len(rows) != expected or len({int(row["seed"]) for row in rows}) != expected:
        raise ValueError("Local terminal token-state seed contract changed")
    rows.sort(key=lambda row: int(row["seed"]))
    plan_core = {
        "schema_version": "realistic_niah_v5_local_terminal_token_state_row_plan_v1",
        "model_label": str(args.model),
        "seed_role": str(args.seed_role),
        "selection_rule": "frozen_outcome_blind_highest_count_anchor_per_seed",
    ...
    if plan_path.exists():
        if json.loads(plan_path.read_text(encoding="utf-8")) != plan:
            raise ValueError("Existing local terminal token-state plan changed")
    else:
    ...
    for index, row in enumerate(rows, start=1):
        shard = shards / f"{_safe_stem(row['request_id'], 'local_terminal_token_state')}.jsonl"
        ...
        completed += 1
        print(f"[local-terminal-token-state] {index}/{len(rows)}", flush=True)
    _atomic_json(
