#!/usr/bin/env python3
"""Run the V6 post-hoc local terminal-token mediation diagnostic."""
...
from realistic_niah_v5.terminal_token_state import (  # noqa: E402
    ...
    run_local_terminal_token_state_bridge_trials,
)
...
def main() -> None:
    ...
    args.command = "local-terminal-token-state-bridge"
    ...
    if int(args.layer) != LAYER_CONTRACTS[str(args.model)]:
        raise ValueError(
            f"V6 local terminal bridge layer contract for {args.model} is "
            f"L{LAYER_CONTRACTS[str(args.model)]}"
        )
    config = V6Config.load(args.v5_config)
    if config.prompt_mode != "enumeration_bullet":
        raise ValueError("The local terminal diagnostic is frozen for Bullet only")
    ...
    if any("selection_rank" in row for row in anchors):
        raise ValueError("V6 local terminal bridge forbids selection_rank")
    if any(
        str(row.get("mode_timing_stratum", timing)) != timing for row in anchors
    ):
        raise ValueError("V6 local terminal registry mixes timing strata")
    anchor_ids = {str(row["request_id"]) for row in anchors}
    if len(anchor_ids) != len(anchors):
        raise ValueError("V6 local terminal registry duplicates requests")
    ...
    if len(rows) != expected or len({int(row["seed"]) for row in rows}) != expected:
        raise ValueError("V6 local terminal bridge requires one row per registered seed")
    rows.sort(key=lambda row: int(row["seed"]))
    plan_core = {
        "schema_version": "realistic_niah_v6_local_terminal_row_plan_v1",
        "analysis_status": DIAGNOSTIC_LABEL,
        "model_label": str(args.model),
        "prompt_mode": config.prompt_mode,
        "mode_timing_stratum": timing,
        "seed_role": str(args.seed_role),
        "selection_rule": "reuse_frozen_outcome_blind_terminal_anchor_per_seed",
        ...
        "conditions": list(REGISTERED_LOCAL_TERMINAL_TOKEN_STATE_CONDITIONS),
        "earlier_trace_items_remain_clean": True,
    }
    plan = {**plan_core, "plan_sha256": _sha256_json(plan_core)}
    plan_path = args.output / "frozen_row_plan.json"
    if plan_path.exists():
        if json.loads(plan_path.read_text(encoding="utf-8")) != plan:
            raise ValueError("Existing V6 local terminal row plan changed")
    else:
    ...
    for index, row in enumerate(rows, start=1):
        shard = shards / f"{_safe_stem(row['request_id'], 'v6_local_terminal')}.jsonl"
        if args.resume and shard.exists():
            skipped += 1
            continue
        results = run_local_terminal_token_state_bridge_trials(
            model,
            tokenizer,
            adapter,
            row,
            layer=int(args.layer),
            random_seed=20260831 + int(row["seed"]),
        ...
        completed += 1
        print(f"[v6 local-terminal-token-state] {index}/{len(rows)}", flush=True)
    _atomic_json(
