...
rows="$(wc -l < "$output/Gemma4-E4B/detail.jsonl")"
if [[ "$rows" -ne "$expected_rows" ]]; then
  echo "Gemma canonical dense row audit failed: $rows != $expected_rows" >&2
  exit 1
fi

cat >"$root/canonical_gemma_dense_complete.json" <<EOF
{
  "status": "complete",
