...
OUTPUT_ROOT="$ROOT_DIR/work/v5_native_count_stream/html_aligned_local_serial_20d10c_20260821_v2/$MODEL"
...
run_phase() {
  ...
  CUDA_VISIBLE_DEVICES="$GPU_INDEX" "$PYTHON" \
    ...
    --output "$trial_root"
  "$PYTHON" \
    "$ROOT_DIR/scripts/analyze_realistic_niah_v5_html_aligned_local_serial.py" \
    --input "$trial_root" \
    --phase "$phase" \
...
value = {
    "schema_version": "realistic_niah_v5_html_local_serial_complete_v1",
    "model_label": model,
    "status": "PASS",
    "completed_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
    "discovery": discovery,
    "confirmation": confirmation,
    "complete_html_local_serial_pass": bool(
        discovery["complete_local_serial_pass"]
        and confirmation["complete_local_serial_pass"]
    ),
}
path = root / "html_local_serial_complete.json"
