...
plan_tsv="${run_root}/orchestration/prompt_revision_shards.tsv"
...
while IFS=$'\t' read -r \
  task_id priority model prompt_mode output_collection analysis_role \
  expected_requests revision
do
  ...
  gpu_memory_utilization="$(
    gpu_memory_utilization_for "${model}"
  )"
  ...
  if (
    ...
    printf "task_id\tmodel\tprompt_mode\tanalysis_role\tgpu_id\tcompleted_at_utc\n" \
      > "${completed_file}"
    printf "%s\t%s\t%s\t%s\t%s\t%s\n" \
      "${task_id}" "${model}" "${prompt_mode}" "${analysis_role}" \
      "${gpu_id}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >> "${completed_file}"
  else
    exit_code=$?
    failed_file="${state_root}/failed/${task_id}.tsv"
    printf "task_id\tmodel\tprompt_mode\tanalysis_role\tgpu_id\texit_code\tfailed_at_utc\n" \
      > "${failed_file}"
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
      "${task_id}" "${model}" "${prompt_mode}" "${analysis_role}" \
      "${gpu_id}" "${exit_code}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >> "${failed_file}"
