...
<section id="summary">
  ...
  <div class="chain-map" role="table" aria-label="Four mechanism chains and their evidence gates">
    ...
    <div class="chain-row" role="row"><div class="chain-name">A · FORM</div><div>Needle 输入先改变完整 span 内的多-token hidden states；这些 states 保存可复用的 occurrence evidence。</div><div>Clean/corrupt span-state contrast 在 restoration forward 中被构造，endpoint running index 可读，但现有报告尚未独立量化该 contrast 的逐层大小与不确定性。</div><div>等长 active-span input corruption 定义 needle→state 干预；固定 corrupt 输入的 clean full-span state patch 直接检验 state→count。</div><div class="chain-status">Arrow A 尚缺独立 state-effect 曲线；Arrow B 已直接验证；局部 endpoint register 不受支持</div></div>
...
<section id="formation">
  ...
  <p class="lead">这里把 hidden state 明确放在逻辑链中间，并把两个箭头分开审计。<strong>箭头 A（needle input→span state）</strong>需要先干预输入，再独立量化同层 span state 发生了多大、是否稳定的变化；现有 restoration runner 已构造 clean/corrupt state contrast，但报告尚未给出专门的逐层 state-effect 曲线与区间，所以这一箭头只能标为<strong>操作已定义、结果未单独闭合</strong>。<strong>箭头 B（span state→count）</strong>则保持 corrupt 输入不变，只把 clean span states 写回网络内部；图 4b 直接显示最终 count 被修复，因此这一箭头已经因果验证。Endpoint PCA 曲线只是这些高维 states 的可视化 readout，不被强行放成必经中介。</p>
  <div class="hidden-state-chain" aria-label="Two-step causal chain from needle input through span hidden states to predicted count">
    ...
    <div class="hidden-state-arrow"><strong>箭头 A · input → state</strong><small>把 active needles 等长替换为 ordinary text；其余 prompt、长度与 query 位置不变。现有运行构造了 H<sup>clean</sup><sub>span</sub> 与 H<sup>corrupt</sup><sub>span</sub>，但尚缺独立的逐层 effect curve。</small></div>
  ...
  <div class="table-wrap"><table><thead><tr><th>逻辑箭头</th><th>目前证据</th><th>因果状态</th><th>严谨结论</th></tr></thead><tbody>
    <tr><td>Needle content → 完整 span hidden states</td><td>同一 seed/count 的 clean forward 与等长 active-needle replacement forward；runner 在同一 span positions 取得两组 residual tensors，供后续 restoration 使用</td><td><strong>部分完成：input intervention 已执行；独立 state-effect 统计尚缺</strong></td><td>两次运行只在 registered needle input positions 不同，因而 <span class="math">ΔH<sub>span</sub><sup>(ℓ)</sup>=H<sub>span</sub><sup>clean</sup>−H<sub>span</sub><sup>corrupt</sup></span> 的方向确由 needle 操作诱发。但现有报告没有把 <span class="math">ΔH</span> 相对 ordinary-region control 的大小逐层汇总、给出 seed-level 区间并在独立 confirmation 上检验；所以不能把箭头 A 写成已经由结果图闭合。</td></tr>
  ...
  <div class="result-grid">
    <div class="result"><span class="value">Arrow A · 尚未单独闭合</span><span class="label">已经做了等长 needle-input replacement，并在 runner 内得到 clean/corrupt span tensors；但目前没有一张独立结果图报告 <span class="math">needle manipulation→ΔH<sub>span</sub></span> 的逐层 effect size、matched ordinary control 与 seed-level uncertainty。</span></div>
  ...
  <div class="formula"><strong>怎样才算把 Arrow A 也直接画出来？</strong>对每个 seed、count 与 layer，预注册完整向量 state-deformation specificity：<span class="math">D<sub>state</sub>(ℓ)=||H<sub>needle</sub><sup>clean</sup>−H<sub>needle</sub><sup>corrupt</sup>||<sub>F</sub>/√(|S|d)−||H<sub>ordinary</sub><sup>clean</sup>−H<sub>ordinary</sub><sup>control</sup>||<sub>F</sub>/√(|S|d)</span>。其中 <span class="math">S</span> 是注册的 span-token 集合，<span class="math">d</span> 是 hidden width；第二项控制“等长输入替换本身会改变多少 state”。随后逐层画 seed-cluster mean 与预注册区间。<span class="example">例：L8 的 needle spans 每个 hidden coordinate 的 RMS change 为 0.42，而等长 ordinary spans 为 0.10，则该单例 <span class="math">D<sub>state</sub>(8)=0.32</span>；只有该差值跨 seeds 稳定为正，才能把 Arrow A 从“干预已执行”升级为“state response 已量化验证”。</span></div>
  <div class="claim"><strong>直接的 hidden-state 因果证据在哪里？</strong><a href="#hidden-state-restoration-evidence"><strong>图 4b dense span restoration</strong></a>直接建立的是 <strong>Arrow B：span state→count</strong>，而不是 Arrow A。先用输入干预产生 <span class="math">H<sup>corrupt</sup></span>；再保持同一 corrupt prompt 不变，仅执行内部 state patch。若最终 expected-count error 相对 ordinary-state patch 明显下降，就说明被写回的完整 span state 对输出具有因果作用。图 4c 是该 state patch 对 downstream retrieval 的响应，也不能代替 Arrow A 所缺的 <span class="math">D<sub>state</sub>(ℓ)</span> 结果。</div>
  <div class="formula"><strong>一个具体例子。</strong>Gold count=6。Clean prompt 的六条 needles 产生 <span class="math">H<sup>clean</sup><sub>span</sub></span>；把六段输入换成等长 ordinary text 后得到 <span class="math">H<sup>corrupt</sup><sub>span</sub></span>，模型倾向答 2。现在不把输入文字改回来，只在 L8 将这些 span positions 的 residual vectors 换成 clean values；若答案回到 5，而同层恢复同样多 ordinary-position vectors 后仍答 2，差异只能归因于<strong>被写回的 needle-dependent hidden-state content</strong>，而不是文本长度、位置或 patch 大小。这一对比直接支持 Arrow B。若还要直接支持 Arrow A，则必须另画 clean/corrupt needle-state change 减 ordinary-control state change，并报告其跨-seed 区间。Endpoint PC1–PC3 即使在图上排列为 1→6，也仍只是这组高维 state 的一个 readout。<strong>目前结论：</strong>完整 span state 对 count 的因果可用性已验证；needle input 对该 state 的特异形变尚待独立量化；“单 endpoint rank-3 counter”不受支持。</div>
  ...
  <div class="claim"><strong>3.3 的直接结果与缺口。</strong><strong>Arrow B</strong> 已在两个模型中直接检测到：完整多-token hidden-state restoration 的行为修复约为 2–3 counts。<strong>Arrow A</strong> 当前只有受控 input manipulation 与 runner 内的 clean/corrupt tensor contrast，尚无独立的 <span class="math">D<sub>state</sub>(ℓ)</span> 结果图，因此不能宣称已由数值结果闭合。另一个不受支持的窄主张是“单 endpoint、线性 rank-3 component 必须被自然使用”。这三点并不矛盾：模型可以依赖分散在多个 token、多个方向或非线性 feature 中的完整 state，而不依赖我们预先选出的三维投影。</div>
